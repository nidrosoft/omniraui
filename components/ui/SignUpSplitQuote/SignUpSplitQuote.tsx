...
import { Sms, Lock, Eye, EyeSlash, ArrowRight, User, Star1 } from "iconsax-react";
import { cn } from "@/lib/cn";
import { Checkbox } from "@/components/ui/Checkbox";
import s from "./SignUpSplitQuote.module.css";

/* ══════════════════════════════════════════════
   Types
   ══════════════════════════════════════════════ */

export interface SignUpSplitQuoteProps {
    /** Product/brand name */
    productName?: string;
    ...
    onSignUp?: (data: { name: string; email: string; password: string; agreedToTerms: boolean }) => Promise<void> | void;
    /** Callback on social login click */
    onSocialLogin?: (provider: "google" | "facebook" | "linkedin") => void;
    /** Callback on sign-in link click */
    onSignInClick?: () => void;
    /** Callback on terms link click */
    onTermsClick?: () => void;
    /** Callback on privacy link click */
    onPrivacyClick?: () => void;
...
/* ══════════════════════════════════════════════
   SignUpSplitQuote Component
   ══════════════════════════════════════════════ */

export function SignUpSplitQuote({
    productName = "Omnira",
    logoText = "O",
    logo,
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await onSignUp?.({ name, email, password, agreedToTerms });
        } catch (err: unknown) {
            setError(err instanceof Error ? err.message : "Sign up failed. Please try again.");
        } finally {
            setLoading(false);
        }
    }, [name, email, password, agreedToTerms, showTerms, onSignUp]);
            ...
            <div className={s.formPanel}>
                <div className={s.formInner}>
                    ...
                    <form className={s.form} onSubmit={handleSubmit}>
                        ...
                        {showTerms && (
                            <div className={s.termsRow}>
                                ...
                                <span>
                                    I agree to the{" "}
                                    <button type="button" className={s.termsLink} onClick={onTermsClick}>Terms of Service</button>
                                    {" "}and{" "}
                                    <button type="button" className={s.termsLink} onClick={onPrivacyClick}>Privacy Policy</button>
                                </span>
                            </div>
                        )}
                    ...
                    {showSocial && (
                        <>
                            ...
                            <div className={s.socialRow}>
                                <button type="button" className={s.socialBtn} onClick={() => onSocialLogin?.("google")} aria-label="Sign up with Google"><GoogleIcon /></button>
                                <button type="button" className={s.socialBtn} onClick={() => onSocialLogin?.("facebook")} aria-label="Sign up with Facebook"><FacebookIcon /></button>
                                <button type="button" className={s.socialBtn} onClick={() => onSocialLogin?.("linkedin")} aria-label="Sign up with LinkedIn"><LinkedInIcon /></button>
                            </div>
                        </>
                    )}
                    ...
                    <div className={s.footer}>
                        ...
                        <button type="button" className={s.footerLink} onClick={onSignInClick}>Sign in</button>
                    </div>
                    ...
                    {starRating > 0 && (
                        <div className={s.starRow}>
                            {Array.from({ length: starRating }).map((_, i) => (
                                <Star1 key={i} size={20} variant="Bold" className={s.star} />
                            ))}
                        </div>
                    )}

                    <p className={s.quoteText}>&ldquo;{quoteText}&rdquo;</p>
